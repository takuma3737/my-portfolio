...
- [ ] 1. プロジェクト基盤と OpenAPI 仕様の作成
  ...
  - Go modules の依存関係を追加（JWT、PostgreSQL、oapi-codegen 等）
  - _Requirements: 全要件_
...
- [ ] 5. JWT 認証システムの実装
  ...
  - [ ] 5.1 JWT 生成・検証機能の実装
    ...
    - ユーザー ID を含む JWT トークンの生成機能を実装する
    - JWT 署名の検証機能を実装する
    - トークン期限切れのハンドリングを実装する
    - _Requirements: 6, 7, 8_

  - [ ] 5.2 認証ミドルウェアの実装
    - HTTP リクエストから JWT を抽出する機能を実装する
    - 認証が必要なエンドポイントでの検証を実装する
    - エラーハンドリングを実装する
    - _Requirements: 6, 7, 8_
...
- [ ] 9. フロントエンド実装
  ...
  - [ ] 9.3 JWT トークン管理の実装
    - ローカルストレージでのトークン保存・取得機能を実装する
    - 初回アクセス時のトークン取得機能を実装する
    - トークン期限切れ時の自動再取得機能を実装する
    - _Requirements: 6, 7, 8_
